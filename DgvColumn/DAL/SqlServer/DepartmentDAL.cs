...
{
    /// <summary>
    /// 对象名称：部门信息SQL Server数据访问子类（数据访问层）
    /// 对象说明：提供“部门信息类（业务逻辑层）”针对SQL Server的“增删改查”等各种数据访问方法，继承通用数据访问父类。
    ...
    public class DepartmentDAL:DAL.Common.DepartmentDAL
    {
        ...
        public override int Insert(Department department)
        {
            ...
            {
                new SqlParameter("@DepartmentName"  , SqlDbType.NVarChar , 50        ){ Value = department.DepartmentName  },
                new SqlParameter("@ManagerName"     , SqlDbType.NVarChar , 50        ){ Value = department.ManagerName     },
                new SqlParameter("@ContactPhone"    , SqlDbType.NVarChar , 50        ){ Value = department.ContactPhone    },
                new SqlParameter("@NoteDescription" , SqlDbType.NText    , 1073741823){ Value = department.NoteDescription }
            };
        ...
        public override int Update(Department department)
        {
            ...
            SqlParameter[] parameters = 
            {
                ...
                new SqlParameter("@ContactPhone"    , SqlDbType.NVarChar , 50        ){ Value = department.ContactPhone    },
                new SqlParameter("@NoteDescription" , SqlDbType.NText    , 1073741823){ Value = department.NoteDescription },
                new SqlParameter("@Id"              , SqlDbType.Int      , 4         ){ Value = department.Id              }
            };
            return SFL.SqlHelper.ExecuteNonQuery(sqlText, parameters);
        }
        ...
        public override Department GetDataById(int id)
        {
            ...
            if (sqlDataReader.Read())
            {
                ...
                ReadDepartmentAllData(sqlDataReader,department);
            }
            sqlDataReader.Close();
            return department;
        }
        ...
        public override PageData GetPageList(int pageSize, int curPage)
        {
            ...
            while (sqlDataReader.Read())
            {
                ...
                {
                    Department department = new Department();
                    ReadDepartmentPageData(sqlDataReader, department);
                    list.Add(department);
                }
            }
            sqlDataReader.Close();
